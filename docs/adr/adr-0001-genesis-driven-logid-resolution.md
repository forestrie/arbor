...
**Decision:** Univocity maintains a forest registry from curator **v1 genesis** documents in the canopy grants bucket (`forest/{hex64(R)}/genesis.cbor`), refreshed on a scan-on-miss circuit breaker. `resolve(logId)` uses genesis identity for `logId == R`, then on-chain `isLogInitialized` probes across registered forests, with ambiguous multi-forest matches returning 503.
